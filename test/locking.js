...
contract('Staking app', () => {
  ...
  const approveStakeAndLock = async(
    _manager,
    _lockAmount = defaultAmount / 2,
    _stakeAmount = defaultAmount,
    _from = owner
  ) => {
    await approveAndStake(_stakeAmount, _from)
    const r = await staking.methods.lock(_lockAmount, _manager, web3.utils.asciiToHex('')).send({ from: _from })
